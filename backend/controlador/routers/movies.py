...
from modelo.config.database import Session
from vista.schemas.movie import Movie
from modelo.models.mensajes import Mensajes
...
def get_movie(id : int):
    ...
    if not result:
        return JSONResponse(status_code=404,content={"message":f"{Mensajes.NOT_FOUND_ID} {id}"})ñ
    return JSONResponse(status_code=200,content=jsonable_encoder(result))
